...
final class Racketmanager_Championship extends RacketManager {
	...
	public function __construct( $league, $settings ) {
		...
		if ( $this->num_groups > 0 ) {
			...
			if ( $this->is_consolation ) {
				...
				if ( 0 === $completed_matches && $num_teams < $this->num_advance ) {
		...
		add_action( 'racketmanager_event_settings_championship', array( &$this, 'championship_settings_page' ) );
	}
	/**
	 * Add settings
	 *
	 * @param event $event competition.
	 */
	public function championship_settings_page( $event ) {
	...
	public function update_final_results( $matches, $home_points, $away_points, $custom, $round, $season ) {
		...
		if ( $round < $this->num_rounds ) {
			$this->proceed( $this->get_final_keys( $round ), $this->get_final_keys( $round + 1 ) );
		}
