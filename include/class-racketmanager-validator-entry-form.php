...
final class Racketmanager_Validator_Entry_Form extends Racketmanager_Validator {
	...
	public function match_day( ?int $match_day, string $field_ref, bool $match_day_restriction = false, array $match_days_allowed = array() ): object {
		if ( empty( $match_day ) && 0 !== $match_day ) {
			...
			$this->err_msgs[$this->error_id] = __( 'Match day not selected', 'racketmanager' );
			++$this->error_id;
		} elseif ( $match_day_restriction ) {
			if ( !empty( $match_days_allowed ) && empty( $match_days_allowed[$match_day] ) ) {
				$this->error = true;
				$this->err_flds[$this->error_id] = 'matchday-' . $field_ref;
				$this->err_msgs[$this->error_id] = __( 'Match day not valid for event', 'racketmanager' );
				++$this->error_id;
			}
		}
		return $this;
	...
	public function match_time( ?string $match_time, string $field_ref, ?string $match_day, array $start_times ): object {
		if ( empty( $match_time ) ) {
			...
			$this->err_msgs[$this->error_id] = __( 'Match time not selected', 'racketmanager' );
			++$this->error_id;
		} elseif ( $match_day >= 0 ) {
			...
			if ( isset( $start_times[$index] ) ) {
				if ( $match_time < $start_times[$index]['min'] ) {
					$this->error = true;
					$this->err_flds[$this->error_id] = 'matchtime-' . $field_ref;
					$this->err_msgs[$this->error_id] = __( 'Match time less than earliest start', 'racketmanager' );
					++$this->error_id;
				} elseif ( $match_time > $start_times[$index]['max'] ) {
					$this->error = true;
					$this->err_flds[$this->error_id] = 'matchtime-' . $field_ref;
					$this->err_msgs[$this->error_id] = __( 'Match time greater than latest start', 'racketmanager' );
					++$this->error_id;
				}
			}
		}
