...
final class Tournament_Service_Test extends TestCase {
    ...
    private function get_service( $tournament_repository ): Tournament_Service {
    ...
    public function test_bulk_remove_tournaments_with_not_found(): void {
        ...
        $service->expects( self::exactly( 2 ) )
            ->method( 'remove_tournament' )
            ->will( self::returnCallback( function( $id ) {
                if ( $id === 123 ) {
                    return 1;
                }
    ...
    private function get_mocked_service( $tournament_repository ): \PHPUnit\Framework\MockObject\MockObject|Tournament_Service {
