...
<div class="container">
	<div class="row justify-content-between mb-3">
		<form id="results-checker-filter" method="get" action="" class="form-control">
			<input type="hidden" name="page" value="<?php echo 'racketmanager-results' ?>" />
			<input type="hidden" name="tab" value="<?php echo 'resultschecker' ?>" />
			<div class="col-auto">
				<select class="form-select-1" size="1" name="season" id="season">
					<option value="all"><?php _e( 'All seasons', 'racketmanager' ) ?></option>
					<?php foreach ( $seasons AS $season ) { ?>
						<option value="<?php echo $season->name ?>" <?php echo $season->name == $seasonSelect ?  'selected' :  '' ?>><?php echo $season->name ?></option>
					<?php } ?>
				</select>
				<select class="form-select-1" size="1" name="competition" id="competition">
					<option value="all"><?php _e( 'All competitions', 'racketmanager' ) ?></option>
					<?php foreach ( $competitions AS $competition ) { ?>
						<option value="<?php echo $competition->id ?>" <?php echo $competition->id == $competitionSelect ?  'selected' :  '' ?>><?php echo $competition->name ?></option>
					<?php } ?>
				</select>
				<select name="filterResultsChecker" size="1">
					<option value="-1" selected="selected"><?php _e('Filter results') ?></option>
					<option value="all" <?php if ( $resultsCheckFilter == 'all' ) echo 'selected' ?>><?php _e('All')?></option>
					<option value="outstanding" <?php if ( $resultsCheckFilter == 'outstanding' ) echo 'selected' ?>><?php _e('Outstanding')?></option>
					<option value="1" <?php if ( $resultsCheckFilter == '1' ) echo 'selected' ?>><?php _e('Approved')?></option>
					<option value="2" <?php if ( $resultsCheckFilter == '2' ) echo 'selected' ?>><?php _e('Handled')?></option>
				</select>
