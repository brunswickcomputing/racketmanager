...
final class RacketManager_Admin_Cup extends RacketManager_Admin {
			...
			if ( $competition ) {
				...
				if ( $season ) {
					if ( isset( $competition->seasons[ $season ] ) ) {
						...
						if ( isset( $cup_season->closing_date ) && $cup_season->closing_date <= gmdate( 'Y-m-d' ) ) {
	...
	public function display_cup_setup_page() {
		if ( ! current_user_can( 'edit_matches' ) ) {
					...
					if ( $competition_id ) {
						...
						if ( $competition ) {
							...
							if ( isset( $_POST['rounds'] ) ) {
								...
								if ( $valid ) {
									$cup_season['matchDates'] = array();
									foreach ( array_reverse( $rounds ) as $match_date ) {
										$cup_season['matchDates'][] = $match_date;
									}
									$cup_seasons                  = $competition->seasons;
									$cup_season['num_match_days'] = count( $cup_season['matchDates'] );
			...
			if ( $competition_id ) {
				$competition = get_competition( $competition_id );
				if ( $competition ) {
					$match_dates = $competition->seasons[ $season ]['matchDates'];
	...
	private function schedule_cup_ratings( $competition_id, $season ) {
		...
		if ( empty( $season->closing_date ) ) {
			...
			$schedule_date  = strtotime( $season->closing_date );
