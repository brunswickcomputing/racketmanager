<?php
/**
 * Tournament Fixtures Admin Controller
 *
...
readonly class Tournament_Fixtures_Admin_Controller {

    public function __construct(
        private Tournament_Fixtures_Admin_Service $fixtures_admin_service,
    ...
    /**
     * Controller for admin.php?page=racketmanager-tournaments&view=fixtures or &view=fixture
     *
     * @param array $query Typically $_GET
     * @param array $post  Typically $_POST
     *
     * @return array{view_model?:Tournament_Fixtures_Page_View_Model, redirect?:string, message?:string, message_type?:bool|string}
     *
     * @throws Tournament_Not_Found_Exception
     */
    public function fixtures_page( array $query, array $post ): array {
        ...
        // POST: manage fixtures (updateLeague=fixture) -> PRG redirect back to GET.
        if ( $is_post ) {
            // Nonce + capability must flow through Action_Guard_Interface.
            $this->action_guard->assert_allowed( 'racketmanager_nonce', 'racketmanager_manage-fixtures', 'edit_matches' );
            ...
            if ( 'fixture' === $view ) {
                $redirect_url = Admin_Redirect_Url_Builder::tournament_fixture( $query, $post, $tournament_id, $league_id, $final_key, $fixture_id );
            } else {
                $redirect_url = Admin_Redirect_Url_Builder::tournament_fixtures( $query, $post, $tournament_id, $league_id, $final_key );
            }
        ...
        // GET: render page
        $vm = $this->fixtures_admin_service->prepare_fixtures_view_model( $tournament_id, $league_id, $final_key, $fixture_id, $view );
    ...
    private function extract_fixture_id( array $query, array $post ): ?int {
        ...
        if ( isset( $post['fixture'][0] ) ) {
            return intval( $post['fixture'][0] );
        }
    ...
    private function extract_view( array $query ): string {
        return isset( $query['view'] ) ? sanitize_text_field( wp_unslash( $query['view'] ) ) : 'fixtures';
    }
}
