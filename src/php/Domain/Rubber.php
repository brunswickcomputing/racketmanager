            ...
            foreach ( $opponents as $opponent ) {
                ...
                foreach ( $players as $player ) {
                    if ( $player ) {
                        ...
                        if ( ! empty( $match->league->event->competition->rules['ageLimitCheck'] ) && ! empty( $options['ageLimitCheck'] ) && ! empty( $match->league->event->age_limit ) && 'open' !== $match->league->event->age_limit ) {
                            if ( empty( $player->age ) ) {
                                ...
                                $age_check = Util::check_age_within_limit( $player_age, $age_limit, $player->gender, $match->league->event->age_offset );
                                if ( ! $age_check->valid ) {
                                    $error = $age_check->msg;
                                    $match->add_player_result_check( $team->id, $player->id, $error, $this->id );
                                }
                        ...
                        if ( isset( $match->match_day ) ) {
                            ...
                            if ( ! empty( $match->league->event->competition->rules['playerLocked'] ) && isset( $options['playerLocked'] ) ) {
