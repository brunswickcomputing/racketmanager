			...
			<div class="col-auto">
				<form action="admin.php" method="get" class="form-control">
					...
					<select size="1" name="season" id="season">
						<?php
						foreach ( $event->seasons as $event->competition_season ) {
							?>
							<option value="<?php echo esc_html( $event->competition_season['name'] ); ?>"
							<?php
							if ( $event->competition_season['name'] === $season['name'] ) {
								echo ' selected="selected"';
							}
							?>
							>
								<?php echo esc_html( $event->competition_season['name'] ); ?>
						</option>
						<?php } ?>
					</select>
		...
		<table class="table table-striped" aria-describedby="<?php esc_html_e( 'Player statistics', 'racketmanager' ); ?>">
			...
			<tbody>
				...
				$playerstats = $event->get_player_stats(
					array(
						'season' => isset( $season['name'] ) ? $season['name'] : false,
						'club'   => $club_id,
					)
				);
				if ( $playerstats ) {
					...
					foreach ( $playerstats as $playerstat ) {
						...
						<tr>
							...
							<td title="Played Down">
								<?php if ( ! 0 === $play_down_count ) { ?>
									<?php echo esc_html( $play_down_count ); ?></td>
								<?php } ?>
							</td>
							<?php
