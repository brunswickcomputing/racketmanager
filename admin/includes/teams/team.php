...
<div class="container league-block">
  <div class="row justify-content-end">
		<div class="col-auto racketmanager_breadcrumb">
			<a href="admin.php?page=racketmanager"><?php _e( 'RacketManager', 'racketmanager' ) ?></a><?php if ( $league ) { ?> &raquo; <a href="admin.php?page=racketmanager&amp;subpage=show-league&amp;league_id=<?php echo $league->id ?>"><?php echo $league->title ?></a><?php } ?> &raquo; <?php echo $form_title ?>
		</div>
	</div>
  <?php if ( $league ) { ?>
  ...
  <?php }
  if ( $league ) {
    $action = "index.php?page=racketmanager&amp;subpage=show-league&amp;league_id=".$league_id."&amp;season=".$season;
  } else {
    $action="admin.php?page=racketmanager-clubs&amp;view=teams";
    if ( $clubId ) {
      $action .= "club_id=".$clubId;
    }
  } ?>
  <form action="<?php echo $action ?>" method="post" enctype="multipart/form-data" name="team_edit" class="form-control">
    ...
    <div class="form-group">
      ...
      <?php if ( $league && $edit ) { ?>
        <input type="hidden" name="affiliatedclub" value="<?php echo $team->affiliatedclub ?>" />
      <?php } ?>
      <div class="input">
        <select size="1" <?php if ( $league && $edit ) { echo 'disabled'; } ?> name="affiliatedclub" id="affiliatedclub" >
    ...
    <div class="form-group">
      ...
      <?php if ( $league && $edit ) { ?>
        <input type="hidden" name="team_type" value="<?php echo $team->type ?>" />
      <?php } ?>
      <div class="input">
        <select size='1' <?php if ( $league && $edit ) { echo 'disabled'; } ?> required="required" name='team_type' id='team_type'>
    ...
    <?php if ( $league ) {
      do_action( 'team_edit_form_'.(isset($league->sport) ? ($league->sport) : '' ), $team );
    } ?>
