<?php
	...
	if ( isset($_POST['updateSettings']) ) {
		...
		$this->editLeague( intval($_POST['league_id']), $_POST['league_title'], $_POST['competition_id'], $settings );
		$this->printMessage();
			...
			<div id='general' class='settings-block-container'>
				...
				<div class="settings-block">
					<table class="lm-form-table">
						...
						<tr valign='top'>
							<th scope='row'><label for='competition_id'><?php _e('Competition', 'leaguemanager') ?></label></th>
							<td>
								<select size='1' name='competition_id' id='competition_id'>
									<option><?php _e( 'Select', 'leaguemanager') ?></option>
<?php if ( $competitions = $leaguemanager->getCompetitions() ) { ?>
<?php foreach ( $competitions AS $competition ) { ?>
									<option value='<?php echo $competition->id ?> '<?php if(isset($league->competition_id)) selected($competition->id, $league->competition_id ) ?>><?php echo $competition->name ?></option>
	<?php } ?>
<?php } ?>
								</select>
							</td>
