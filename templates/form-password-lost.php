<div id="password-lost-form" class="login-form-container widecolumn">
    <?php if ( $vars['show_title'] ) { ?>
        <h3><?php _e( 'Forgot Your Password?', 'leaguemanager' ); ?></h3>
    <?php } ?>
    ...
    <form id="lostpasswordform" action="<?php echo wp_lostpassword_url(); ?>" method="post">
        <p class="form-row">
            ...
            <input type="email" placeholder="<?php _e( 'Email', 'leaguemanager' ); ?>" name="user_login" id="user_login">
        </p>
        <p class="lostpassword-submit">
            <input type="submit" name="submit" class="lostpassword-button"
                   value="<?php _e( 'Reset Password', 'leaguemanager' ); ?>"/>
        </p>
    </form>
</div>
