...
class League_Tennis extends League {
	...
	*/
	protected function getStandingsData( $team_id, $data = array(), $matches = false ) {
		...
		if ( !$matches ) {
			$matches = $league->getMatches( array("season" => $season, "team_id" => $team_id, "final" => '', "limit" => false, "cache" => false, "home_points" => 'not null', "away_points" => 'not null', 'reset_query_args' => true, "confirmed" => true) );
		}
