...
class RacketManager {
	...
	public function set_page_title( $title ) {
		...
		if ( 'tournament' === $slug ) {
			$tournament = un_seo_url( isset( $wp->query_vars['tournament'] ) ? ucwords( $wp->query_vars['tournament'] ) : __( 'Tournament', 'racketmanager' ) );
	 ...
	 */
	public function notify_favourites( $matches, $league ) {
		$users = $this->get_users_for_favourite( 'league', $league->id );
		if ( $users ) {
			$favourite = $league->title;
			foreach ( $matches as $m => $match_id ) {
				$match         = get_match( $match_id );
				$matches[ $m ] = $match;
			}
			$this->notify_favourites_email( $favourite, $league, $users, $matches );
		}
		...
		foreach ( $matches as $i => $match_id ) {
			...
			if ( isset( $match->teams['home']->affiliatedclub ) ) {
				...
				$clubs[ $i ]['name']      = $match->teams['home']->club->shortcode;
			...
			if ( isset( $match->teams['away']->affiliatedclub ) && isset( $match->teams['home']->affiliatedclub ) && $match->teams['home']->affiliatedclub !== $match->teams['away']->affiliatedclub ) {
				...
				$clubs[ $i ]['name']      = $match->teams['away']->club->shortcode;
				$clubs[ $i ]['matches']   = array();
				$clubs[ $i ]['matches'][] = $match;
			}
		}
		...
		foreach ( $clubs as $club ) {
			$users = $this->get_users_for_favourite( 'club', $club['id'] );
			if ( $users ) {
				$favourite = $club['name'];
				$this->notify_favourites_email( $favourite, $league, $users, $club['matches'] );
			}
		}
	}
	 ...
	 */
	public function get_users_for_favourite( $type, $key ) {
		return get_users(
			array(
	 ...
	 */
	public function notify_favourites_email( $favourite, $league, $users, $matches ) {
		global $racketmanager_shortcodes;

		$headers           = array();
		$from_email        = $this->get_confirmation_email( $league->event->competition->type );
		$headers[]         = 'From: ' . ucfirst( $league->event->competition->type ) . ' Secretary <' . $from_email . '>';
		$organisation_name = $this->site_name;
		$email_subject     = $this->site_name . ' - ' . $league->title . ' Result Notification';
		$favourite_url     = $this->site_url . '/member-account/favourites';
		$match_url         = $this->site_url . '/' . $league->event->competition->type . '/' . seo_url( $league->title ) . '/' . $league->current_season['name'] . '/';
