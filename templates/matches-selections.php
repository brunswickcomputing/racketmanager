...
<?php if ( ($league->show_match_day_selection || $league->show_team_selection) && $league->mode != 'championship' ) { ?>
	<div class="matches-selections wp-clearfix" style='float: left; margin-top: 1em;'>
		<form method='get' action='<?php the_permalink(get_the_ID()); ?>' id='leaguemanager_match_day_selection'>
		<div>
	...
	<?php if ($league->show_match_day_selection) { ?>
			<select size="1" name="match_day" id="match_day">
				<option value="-1"<?php selected(get_current_match_day(), -1) ?>><?php _e( 'Show all Matches', 'leaguemanager' ) ?></option>
		<?php for ($i = 1; $i <= $league->num_match_days; $i++) { ?>
				<option value='<?php echo $i ?>'<?php selected(get_current_match_day(), $i) ?>><?php printf(__( '%d. Match Day', 'leaguemanager'), $i) ?></option>
		<?php } ?>
			</select>
	<?php } ?>
