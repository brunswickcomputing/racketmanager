...
<div class="container league-block">
	...
	<form action="/wp-admin/admin.php?page=racketmanager-clubs<?php echo empty( $club_id ) ? null : '&amp;club_id' . esc_html( $club_id ); ?>" method="post" enctype="multipart/form-data" name="club_edit" class="form-control">
        ...
        if ( $edit ) {
            ?>
			<div class="form-floating mb-3">
				<input type="text" class="form-control" name="match_secretary_name" id="match_secretary_name" autocomplete="off" value="<?php echo esc_html( $club->match_secretary_name ); ?>" size="40" /><input type="hidden" name="match_secretary" id="match_secretary" value="<?php echo esc_html( $club->matchsecretary ); ?>" />
