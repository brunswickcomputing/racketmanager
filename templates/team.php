...
<?php if ( $league->mode != 'championship' ) { ?>
        <div style='float: right; margin-top: 1em;'>
            <a href="/index.php?league_id=<?php echo $league->id ?>&team_id=<?php echo the_team_id() ?>&team=<?php echo the_team_name() ?>&season=<?php echo $season ?>&leaguemanager_export=calendarFile" class="roll-button calendar-add" title="<?php _e( 'Add Matches to Calendar', 'leaguemanager' ) ?>" >
                <i class="leaguemanager-svg-icon">
                    <?php leaguemanager_the_svg('icon-calendar') ?>
                </i>
            </a>
        </div>
<?php } ?>
...
<?php
    if ( !empty($teaminfo->captain) ) { ?>
			<dt><?php _e( 'Captain', 'leaguemanager' ) ?></dt><dd><?php echo $teaminfo->captain ?></dd>
<?php }
    if ( is_user_logged_in() ) {
        if ( !empty($teaminfo->contactno) ) { ?>
			<dt><?php _e( 'Contact Number', 'leaguemanager' ) ?></dt><dd><?php echo $teaminfo->contactno ?></dd>
    <?php }
        if ( !empty($teaminfo->contactemail) ) { ?>
			<dt><?php _e( 'Contact Email', 'leaguemanager' ) ?></dt><dd><?php echo $teaminfo->contactemail ?></dd>
    <?php }
    } else { ?>
    ...
    if ( !empty($teaminfo->match_day) ) { ?>
			<dt><?php _e( 'Match Day', 'leaguemanager' ) ?></dt><dd><?php echo $teaminfo->match_day ?></dd>
<?php }
    if ( !empty($teaminfo->match_time) && $teaminfo->match_time > "00:00:00" ) { ?>
			<dt><?php _e( 'Match Time', 'leaguemanager' ) ?></dt><dd><?php echo $teaminfo->match_time ?></dd>
<?php } ?>
        </dl>
			...
			<div class="matches-container">
                <div class="next_match">
                    ...
                    <div class="content">
                        ...
                        <p class='match_day'><?php printf(__("<strong>%d.</strong> Match Day", 'leaguemanager'), get_match_day()); ?></p>
